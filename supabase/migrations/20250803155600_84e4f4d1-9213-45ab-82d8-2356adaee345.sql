...
-- and addressing security definer concerns

-- Update authenticate_admin function with proper security settings
...
-- Update hash_password function with proper security settings
...
-- Update validate_admin_session function with proper security settings
...
-- Update create_admin_session function with enhanced security
...
-- Update invalidate_admin_session function with proper security
...
-- Add last_accessed column to admin_sessions if not exists
DO $$ 
BEGIN
    IF NOT EXISTS (
...
CREATE INDEX IF NOT EXISTS idx_admin_sessions_active_token ON admin_sessions(token) WHERE is_active = true;
